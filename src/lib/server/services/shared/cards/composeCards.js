...
export async function fetchListItems({ db, type, lang = "en" }) {
    ...
    if (listCfg.sortOptions?.default)
        pipeline.push({ $sort: listCfg.sortOptions.default });

    if (listCfg.sample) pipeline.push({ $sample: { size: listCfg.sample } });

    pipeline.push({ $limit: listCfg.limit ?? 100 });
    pipeline.push({ $project: projection });
    ...
    if (withReviews) {
        ...
        items = items.map((i) => ({
            ...i,
            reviewsCount: map[i.slug]?.reviewsCount ?? 0,
            rating: map[i.slug]?.rating ?? 0,
        }));
    }
