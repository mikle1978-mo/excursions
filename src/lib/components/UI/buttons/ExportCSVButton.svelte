<script>
    ...
    export let label = "Скачать CSV";
    export let variant = "primary"; // 'primary' | 'secondary' | 'outline'
    export let size = "md"; // 'sm' | 'md' | 'lg'

    function handleClick() {
        if (confirm("Скачать таблицу?")) {
            exportToCSV(items, fileName, columnMap);
        }
    }
</script>

<button class="custom-button {variant} {size}" on:click={handleClick}>
    {label}
</button>
