<script>
    ...
    import { createItem, updateItem } from "$lib/client/itemsActions.js";
    import { collectionsConfig } from "$lib/config/app.config.js";

    export let mode = "create"; // create | edit
    export let item = null;
    let initialData = item ? structuredClone(item) : null;
    ...
    const collection = collectionsConfig[type];
    const schema = collection.schema;
    const formConfig = collection.formConfig;

    let steps = collection.formConfig ?? [];
